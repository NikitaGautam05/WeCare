...
//     // 1. Get the Caregiver ID stored during login
//     const caregiverId = localStorage.getItem("caregiverId");

//     if (caregiverId) {
//       // 2. Navigate to the ID-specific route
//       navigate(`/CareGiverDash/${caregiverId}`);
//     } else {
//       // Fallback if ID is missing (e.g., session expired)
//       console.error("Caregiver ID not found");
//       navigate("/login");
//     }
//   };
//   return (
...
const Welcome = () => {
  ...
  const handleContinue = () => {
    ...
    if (userId) {
      ...
      navigate("/CareGiverDash/:id");
    } else {
      console.warn("No session found, heading to login.");
      navigate("/login");
    }
  };
